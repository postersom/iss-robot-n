...
function viewall(ip_core, serialnumber, test_id, num_slot, slot) {

	slot = "A01";
	var ip_local = $('#ip_local').val();

	$.ajax({
		url: "http://" + ip_local + ":8008/getList?ip_core=" + ip_core + "&sn=" + serialnumber + "&test_id=" + test_id + "&location=" + slot
		...
		for (var i = 0; i < num_slot; i++) {
			uutData = uutData + '<div id="box' + (i + 1) + '"><div class="col-sm-3" style="padding:0;color:#444">' +
				'<div class="panel" style="width:100%;margin-bottom:0;">' +
				'<div class="panel-heading" style="background-color:#CCCCCC;border-radius:0px !important;">' +
				'<div class="row">' +
				'<div class="col-md-6" style="font-size:24px;font-weight:bold;" id="' + (i + 1) + '">' + (i + 1) + '</div>' +
				'<div class="col-md-6" style="font-size:12px;font-weight:bold;text-align:right;">' +
				'<span style="font-weight: bold;font-size:12px;">-</span><br><span style="font-weight: normal;font-size:12px;">-</span>' +
				'</div>' +
				'</div>' +
				'</div>' +
				' <div class="panel-body" style="background-color:#f5f5f5 ; padding:0; border-left: 3px solid #EFEFEF; border-bottom: 1px solid #fff; border-top: 3px solid #f5f5f5; border-style: 3px solid; border-radius: 3px;">' +
				'<div style="text-align:left;">' +
				'<span style="font-size:16px;margin:5px;color:#EFEFEF;"><b>-</a></b><span id="no' + i + '"></span>' +
				'</div>' +
				'<div style="font-size:10px;" width="18" height="18"><p style="text-align:left;font-size:12px;margin:5px;color:#000;float:center;">-</p></div>' +
				'</div>' +
				'<div class="panel-footer" style="background-color:#f5f5f5 ; padding:0; border-left: 3px solid #EFEFEF; border-bottom: 3px solid #f5f5f5; border-top: 1px solid #fff; border-style: 3px solid; border-radius: 3px;"><a class="comment" style="display:block; text-align:left; color:#444; margin:5px;font-size:11px; padding:0;" data-toggle="tooltip" data-placement="bottom" title="-">-</a>' +
				'</div>' +
				'</div>' +
				'</div>' +
				'</div>' +
		...
		$.each(data, function (key, value) {
			...
			if (status == "abort" || status == "Aborted") {
				color_h = "#D88200"; color_b = "#FF9900";
			} else if (status == "fail" || status == "Failed") {
				color_h = "#C40101"; color_b = "#FF0000";
			} else if (status == "failing" || status == "Failing") {
				color_h = "#660020"; color_b = "#990033";
			} else if (status == "testing" || status == "Testing") {
				color_h = "#EFC700"; color_b = "#EFC700";
			} else if (status == "Passes") {
				color_h = "#007727"; color_b = "#009933";
			} else if (status == "Scanning") {
				color_h = "#B2C24E"; color_b = "#B2C24E";
			} else if (status == "Deleting" || status == "Checking out") {
				color_h = "#3DA8CC"; color_b = "#3DA8CC";
			} else if (status == "Aborting") {
				color_h = "#CC803D"; color_b = "#CC803D";
			} else if (status == "New") {
				color_h = "#CCCCCC"; color_b = "#EFEFEF";
			} else if (status == "-") {
				color_h = "#CCCCCC"; color_b = "#EFEFEF";
			}
			...
			$("#box" + (parseInt(key, 10) + 1)).html(uutData);
